...
def get_contratante(db: Session, id_contratante: int):
    return db.query(Contratante).filter(Contratante.CPF == id_contratante).first()

def update_contratante(db: Session, id_contratante: int, contratante: ContratanteUpdate):
    db_contratante = db.query(Contratante).filter(Contratante.CPF == id_contratante).first()
    if db_contratante:
